...
Section PBFTdelay_of_send_commits.
  ...
  Lemma delay_of_send_commits :
    forall (eo : EventOrdering) (e : Event) p dst delay,
      In (MkDMsg (PBFTcommit p) dst delay) (output_system_on_event_ldata PBFTsys e)
      -> delay = 0.
  Proof.
    introv j.
...

    {
      ...
      - apply in_check_broadcast_commit_implies in j; unfold send_commit in *; exrepnd; ginv.
...

    {
      ...
      - apply send_message_in_trim_outputs_with_low_water_mark in j.
        ...
        repndors; exrepnd; unfold send_prepare, send_commit in *; ginv.
      - eapply message_in_update_state_new_view_implies in j;[|eauto]; exrepnd; ginv.
    }
  Qed.

  Lemma send_commit_no_delay :
    forall (eo : EventOrdering) (e : Event) p dst delay,
      In (MkDMsg (PBFTcommit p) dst delay) (output_system_on_event_ldata PBFTsys e)
      -> In (send_commit p dst) (output_system_on_event_ldata PBFTsys e).
  Proof.
    introv i.
    applydup delay_of_send_commits in i; subst; auto.
  Qed.

End PBFTdelay_of_send_commits.
