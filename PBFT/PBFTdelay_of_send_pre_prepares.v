...
Section PBFTdelay_of_send_pre_prepares.
  ...
  Lemma delay_of_send_pre_prepares :
    forall (eo : EventOrdering) (e : Event) p dst delay,
      In (MkDMsg (PBFTpre_prepare p) dst delay) (output_system_on_event_ldata PBFTsys e)
      -> delay = 0.
  Proof.
    introv j.
    ...
    destruct m; simpl in *; ginv; subst; tcsp;
      smash_handlers; try conflicting_sends;
        ...
        try (complete (unfold broadcast2others in *; unfold send_pre_prepare in *; ginv)).
  ...
  Lemma send_pre_prepare_no_delay :
    forall (eo : EventOrdering) (e : Event) p dst delay,
      In (MkDMsg (PBFTpre_prepare p) dst delay) (output_system_on_event_ldata PBFTsys e)
      -> In (send_pre_prepare p dst) (output_system_on_event_ldata PBFTsys e).
  Proof.
    introv i.
    applydup delay_of_send_pre_prepares in i; subst; auto.
  Qed.

End PBFTdelay_of_send_pre_prepares.
