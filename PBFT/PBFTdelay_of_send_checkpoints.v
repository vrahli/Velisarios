...
Section PBFTdelay_of_send_checkpoints.
  ...
  Lemma delay_of_send_checkpoints :
    forall (eo : EventOrdering) (e : Event) p dst delay,
      In (MkDMsg (PBFTcheckpoint p) dst delay) (output_system_on_event_ldata PBFTsys e)
      -> delay = 0.
  Proof.
    introv j.
    ...
    {
      (* check-ready *)
      eapply message_in_find_and_execute_requests_implies in j;[|eauto]; repndors; exrepnd; unfold send_checkpoint in *; ginv.
    }

    {
      (* check-bcast-new-view *)
      repndors; ginv.
      eapply message_in_update_state_new_view_implies in j;[|eauto]; exrepnd; unfold send_checkpoint in *; ginv.
    }
  ...
  Lemma send_checkpoint_no_delay :
    forall (eo : EventOrdering) (e : Event) p dst delay,
      In (MkDMsg (PBFTcheckpoint p) dst delay) (output_system_on_event_ldata PBFTsys e)
      -> In (send_checkpoint p dst) (output_system_on_event_ldata PBFTsys e).
  Proof.
    introv i.
    applydup delay_of_send_checkpoints in i; subst; auto.
  Qed.

End PBFTdelay_of_send_checkpoints.
