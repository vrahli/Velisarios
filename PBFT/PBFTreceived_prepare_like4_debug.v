...
Section PBFTreceived_prepare_like4.
  ...
  Lemma send_pre_prepares_are_in_log :
    forall (eo : EventOrdering) (e : Event) p dst st i,
      (*loc e = PBFTreplica i
      ->*) In (send_pre_prepare p dst) (output_system_on_event_ldata PBFTsys eo e)
  ...
  Proof.
    ...
    remember (loc e) as n; destruct n;
      try (complete (apply MhaltedSM_output in j; tcsp)).
    ...
    match goal with
    | [ H : context[state_sm_before_event ?a ?b ?c] |- _ ] =>
      remember (state_sm_before_event a b c) as q; symmetry in Heqq; hide_hyp Heqq
    end.
    destruct q; simpl in *; ginv;[].

    remember (trigger e) as trig; symmetry in Heqtrig.
    unfold PBFTreplica_update in *.
    destruct trig; simpl in *; ginv; subst; tcsp;
