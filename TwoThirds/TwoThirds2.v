...
This is an alternative definition of 2/3 consensus from the one defined in TwoThirds.v,
that's turned upside down, i.e., where we start from the event recognizers.
...
Require Import Parallel.
Require Import Map.
Require Import Bind.
Require Import Until.
Require Import SendId.
...
Section TwoThirdConsensus.
  ...
  Definition nat_n (n : nat) : Type := {m : nat | m <? n = true}.
  ...
  | TTCretry   (b : Ballot)
  | TTCnewProposal (p : Proposal).

  Instance TTC_I_Msg : Msg.
  Proof.
    exact (Build_Msg TTCmsg).
  Defined.
  ...
  (* ===============================================================
     Receive functions
     =============================================================== *)
  Definition receive_proposal (m : TTCmsg) : option Proposal :=
    match m with
    | TTCpropose p => Some p
  ...
  Definition TTCreceiveProposal : StateMachine _ TTCmsg (option Proposal) :=
    mkSSM (fun state m => (state, receive_proposal m)) tt.

  Definition receive_new_proposal (m : TTCmsg) : option Proposal :=
    match m with
    | TTCnewProposal p => Some p
    | _                => None
    end.

  Definition TTCreceiveNewProposal : StateMachine _ TTCmsg (option Proposal) :=
    mkSSM (fun state m => (state, receive_new_proposal m)) tt.
  ...
  Definition TTCquorumState : Type := list Cmd * list Rep.
  ...
  (* NOTE: Trying to move out some components to run them in parallel *)
  Definition Handler : NMStateMachine _ :=
    (nmapSM
       (fun slf pop =>
          match pop with
          | Some p => [MkDMsg slf (TTCnewProposal p)]
          | None => []
          end)
       (fun _ => TTCnewVotersOpt))  (* QUESTION? why is the coercion not used here? *)
    [||]
    (TTCreceiveNewProposal [>>=] TTCvoter).
...
*)
  ...
  Definition TTCreplicaState : Type :=
    bindState
  ...
  Instance TTC_I_Key : Key.
  Proof.
    exact (Build_Key unit).
  Defined.

  Instance TTC_I_Data : Data.
  Proof.
    exact (Build_Data TTCmsg).
  Defined.

  Instance TTC_I_Auth : Auth.
  Proof.
    exists unit.
    - exact Deq_unit.
    - exact (fun _ _ => tt).
  ...
  Instance TTC_I_DataAuth : DataAuth.
  Proof.
    exact (Build_DataAuth TTCdata_auth).
  Defined.

  Instance TTC_I_ContainedAuthData : ContainedAuthData.
  Proof.
    exact (Build_ContainedAuthData
  ...
  Definition TTChold_keys (eo : EventOrdering) : Prop :=
    forall (e : Event),
      match loc e with
      | TTCreplica n => forall m, has_key (keys e) (TTCreplica m)
      | _ => True
      end.
  ...
  Lemma TTCreceivedProposal_output_iff :
    forall (eo : EventOrdering) (e : Event) p,
      output_sm_on_event TTCreceivedProposal eo e = Some (Some p)
      <->
      (
      ...
      repndors; exrepnd; subst; destruct (dec_isFirst e); allrw; simpl in *; auto;
        ...
        | [ |- context[option_map _ ?x] ] => remember x as sop
          ...
          assert False; tcsp; unfold TTCreceivedProposal in Heqsop;
            match goal with
            | [ H : context[_ (mkSSM ?u ?i) ?eo ?e] |- _ ] =>
              pose proof (state_sm_on_event_mkSSM eo e u i) as xx
  ...
  Lemma TTCnewVotersOpt_valid_list_with_missing :
    forall (eo : EventOrdering) (e : Event) lwm,
      SM_state_before_event TTCnewVotersOpt eo e lwm
      -> valid_list_with_missing lwm.
  Proof.
    intro eo; induction e as [e ind] using CausalOrderInd; introv sm.
  ...
  Lemma SM_state_before_event_implies_TTCnewVotersOpt_not_in_implies :
    forall (eo : EventOrdering) (e : Event) lwm p,
      ...
      -> output_sm_on_event TTCreceivedProposal eo e = Some (Some p)
      -> ~ in_list_with_missing (proposal2slot p) lwm
      -> SM_state_on_event TTCnewVotersOpt eo e (add_proposal p lwm).
  Proof.
  ...
  Lemma TTCnewVoters_output_iff :
    forall (eo : EventOrdering) e (p : Proposal),
      In p (loutput_sm_on_event TTCnewVoters eo e)
      <->
      exists lwm,
        SM_state_before_event TTCnewVotersOpt eo e lwm
        /\ valid_list_with_missing lwm
  ...
  Lemma output_sm_on_event_TTCreceiveVote_iff :
    forall (eo : EventOrdering) (e : Event) v,
      output_sm_on_event TTCreceiveVote eo e = Some (Some v)
      <-> trigger e = TTCvote v.
  Proof.
  ...
  Lemma output_sm_on_event_TTCreceiveDecided_iff :
    forall (eo : EventOrdering) (e : Event) v,
      output_sm_on_event TTCreceiveDecided eo e = Some (Some v)
  ...
  Lemma output_sm_on_event_TTCroundInfo_iff :
    forall (eo : EventOrdering) (e : Event) b,
      output_sm_on_event TTCroundInfo eo e = Some (Some b)
      <->
      (
  ...
  Lemma TTCsys_output_iff :
    forall (eo : EventOrdering) e l m,
      In (MkDMsg l m) (output_system_on_event_ldata TTCsys eo e)
      <->
      (
        (
          exists (n : Rep) (e' : Event) (p : e' ⊑ e) (sn : SlotNum) (rn : RoundNum)
            ...
            /\ valid_list_with_missing lwm
            /\ no_loutput_sm_on_event_prior_to TTCsendNotify (subEventOrdering e') (mkSubOrderingLe p)
            /\
            (
            ...
            (
              ...
              exists (r0 : RoundNum),
                r0 < rn
                /\
                SM_state_before_event (TTCnewRounds sn) (subEventOrdering e') (mkSubOrderingLe p) r0
                /\
                (
        ...
        (
          exists (n : Rep) (e' : Event) (p : e' ⊑ e) (sn : SlotNum) (rn : RoundNum)
            ...
            /\ no_loutput_sm_on_event_prior_to TTCsendNotify (subEventOrdering e') (mkSubOrderingLe p)
            /\ SM_state_before_event (TTCQuorum (vround sn rn) (TTCreplica n)) (subEventOrdering e'') (mkSubOrderingLe p') (cmds, names)
            ...
            (
              ...
              exists (rn0 : RoundNum) (cmd0 : Cmd),
                ...
                /\ rn0 < rn
                /\ SM_state_before_event (TTCnewRounds sn) (subEventOrdering e') e'' rn0
                /\ trigger e = TTCvote (vote (ballot (vround sn rn) cmd) sender)
            )
        )
        ...
        (
          exists (n : Rep) (e' : Event) (p : e' ⊑ e) (p1 p2 : Proposal)
            ...
            /\ state_sm_before_event_exists (TTCrounds p1 (TTCreplica n)) (subEventOrdering e') (mkSubOrderingLe p)
            /\ In l nclients
        )
      ).
  Proof.
    ...
    - unfold TTCsys in h.
      ...
      + unfold TTCreplicaSM in h; simpl in h.
        ...
        pose proof (output_bind_iff
                      ...
                      eo e
                      (MkDMsg l m)) as q; apply q in h; clear q.
        exrepnd.
        ...
        * unfold TTCrounds in h0; simpl in *.
...

          {
            ...
            - apply SendId_list_output_iff in h0; repnd.
              ...
              destruct t.
              left.
              exists n e p sn (rnum 0) cmd cmd lwm; simpl.
              dands; auto.
            ...
            - unfold TTCQuorum in h0.
              ...
              + apply in_map_iff in h0; exrepnd.
                ...
                exists (mkSubOrderingLe (localCausalOrderLe_refl e')) p.
                dands; auto.
...

          {
            ...
            - apply SendId_list_output_iff in h6; repnd.
              ...
              left.
              exists n e' p (snum n2) (rnum n3) cmd0 cmd lwm.
              dands; auto.
            ...
            - unfold TTCQuorum in h6.
              ...
              + apply in_map_iff in h6; exrepnd.
                ...
                assert (e'0 ⊑ e) as q.
                { dup p0 as xx; apply localCausalOrderLe_subEventOrdering_implies in xx; simpl in xx; auto. }
    ...
    - unfold TTCsys.
      ...
      pose proof (output_bind_iff
                    ...
                    (fun t : Proposal => TTCvoter t (TTCreplica n))
                    eo e
                    (MkDMsg l m)) as q; apply q; clear q.
...

          {
            ...
            assert
              (@localCausalOrderLe
                 _ _ _
                 (subEventOrdering e')
                 (mkSubOrderingLe (localCausalOrderLe_refl e'))
                 (mkSubOrderingLe p)) as q.
            { apply localCausalOrderLe_subEventOrdering_iff; simpl; auto. }

            exists
              (mkSubOrderingLe p)
              (@localCausalOrderLe_refl _ _ _ (subEventOrdering e') (mkSubOrderingLe p))
      ...
      + exists e' p (prop sn cmd0).
        ...
        * unfold TTCvoter.
...

          {
            ...
            - revert p' h8; rewrite h1; introv h.
              rewrite <- Heqn.
...

          {
            ...
            assert (@localCausalOrderLe _ _ _ (subEventOrdering e') e'' (mkSubOrderingLe p)) as q.
            { apply localCausalOrderLe_subEventOrdering_iff; simpl; auto. }
      ...
      + exists e' p (prop sn cmd0).
        ...
        * unfold TTCvoter.
...

          {
            ...
            - revert p' h9; rewrite h0; introv h.
              rewrite <- Heqn.
...

          {
            ...
            - unfold TTCround; simpl.
              ...
              + rewrite <- Heqn.
                rewrite SM_state_before_event_sub_sub_so_as_sub_eo.
                { erewrite sub_sub_event2sub_event_mkSubEventOrderingLe; exact h9. }
                simpl in *; auto.
                dup p' as xx; apply localLe_implies_loc in xx; simpl in *; auto.
      ...
      + exists e' p p1.
        ...
        * unfold TTCvoter.
          ...
          { apply in_map_iff; exists l; dands; auto. }
  Qed.

  Lemma TTCvote_iff :
    forall (eo : EventOrdering) e l v,
      In (MkDMsg l (TTCvote v)) (output_system_on_event_ldata TTCsys eo e)
      <->
      exists (n : Rep) (e' : Event) (p : e' ⊑ e) (sn : SlotNum) (rn : RoundNum)
             (cmd cmd0 : Cmd) (lwm : list_with_missing),
  ...
  Proof.
    ...
    - repndors; auto; exrepnd; ginv;[].
      exists n e' p sn rn cmd cmd0 lwm; dands; auto.
  ...
  Lemma TTCdecided_iff :
    forall (eo : EventOrdering) e l prp,
      In (MkDMsg l (TTCdecided prp)) (output_system_on_event_ldata TTCsys eo e)
      <->
      exists (n : Rep) (e' : Event) (p : e' ⊑ e) (sn : SlotNum) (rn : RoundNum)
             (cmd0 : Cmd) (lwm : list_with_missing) (cmd : Cmd) (sender : Rep)
             (cmds : list Cmd) (names : list Rep) (c : Cmd)
  ...
  Proof.
    ...
    - repndors; auto; exrepnd; ginv;[].
      exists n e' p sn rn cmd0 lwm cmd sender cmds names c e'' p'; dands; auto.
  ...
  Lemma TTCretry_iff :
    forall (eo : EventOrdering) e l b,
      In (MkDMsg l (TTCretry b)) (output_system_on_event_ldata TTCsys eo e)
      <->
      exists (n : Rep) (e' : Event) (p : e' ⊑ e) (sn : SlotNum) (rn : RoundNum)
             (cmd0 : Cmd) (lwm : list_with_missing) (cmd : Cmd) (sender : Rep)
             (cmds : list Cmd) (names : list Rep) (k : nat) (c : Cmd)
  ...
  Lemma TTCnotify_iff :
    forall (eo : EventOrdering) e l p2,
      In (MkDMsg l (TTCnotify p2)) (output_system_on_event_ldata TTCsys eo e)
      <->
      exists (n : Rep) (e' : Event) (p : e' ⊑ e) (p1 : Proposal)
             (lwm : list_with_missing),
        loc e = TTCreplica n
        /\ SM_state_before_event TTCnewVotersOpt eo e' lwm
        /\ ~ in_list_with_missing (proposal2slot p1) lwm
  ...
  Proof.
    ...
    - repndors; auto; exrepnd; ginv;[].
      exists n e' p p1 lwm; dands; auto.

    - right; right; right.
      exrepnd.
      exists n e' p p1 p2 lwm; dands; auto.
  Qed.

  Lemma TTCpropose_iff :
    forall (eo : EventOrdering) e l p,
      In (MkDMsg l (TTCpropose p)) (output_system_on_event_ldata TTCsys eo e)
      <->
      False.
  Proof.
    introv.
  ...
  Lemma TTCkey_replicas :
    forall (eo : EventOrdering) e n m,
      loc e = TTCreplica n
      -> TTChold_keys eo
      -> {k : key & lookup_key (keys e) (TTCreplica m) = Some k}.
  Proof.
    introv lp hk.
    ...
    unfold has_key in h.
    unfold lookup_key.
    remember (lookup_dkey (keys e) (TTCreplica m)) as lk; destruct lk; simpl; tcsp; GC.
  ...
  Lemma in_quorum_from_vote :
    forall (eo : EventOrdering) (e : Event) vr slf cmds names cmd,
      SM_state_before_event (TTCQuorum vr slf) eo e (cmds, names)
      -> In cmd cmds
      -> exists e' sender,
          e' ⊑ e
  ...
  Proof.
    introv.
    revert cmds names.
    induction e as [e ind] using CausalOrderInd; introv sm i.
    ...
    remember (state_sm_before_event (TTCQuorum vr slf) eo (local_pred e)) as sop.
  ...
  Lemma TTCsent :
    forall (eo : EventOrdering) (e : Event) r1 r2,
      ...
      -> exists (e' : Event) dst,
          e' ≺ e
          /\ In (MkDMsg dst (trigger e)) (output_system_on_event_ldata TTCsys eo e').
  Proof.
  ...
  Lemma TTCvalidity_aux :
    forall (eo : EventOrdering),
      ...
      -> TTCall_not_byz eo
      -> TTChold_keys eo
      -> forall (e : Event) (p : Proposal) (c : name) (m : msg),
          In (MkDMsg c m) (output_system_on_event_ldata TTCsys eo e)
          ->
          (
            m = TTCpropose p
            \/
  ...
  Proof.
    introv byz nbyz hk.
    induction e as [e ind] using CausalOrderInd; introv i eqm.
    ...
    - apply TTCnotify_iff in i; exrepnd.
      ...
      apply (ind e'0 M1h0 p) in M1h1; tcsp.
    ...
    - exrepnd; subst.
      ...
      + destruct i7 as [i7|i7].
        ...
        * exrepnd.
          ...
          apply (ind e' M1h0 (prop sn cmd)) in M1h1;
      ...
      + exrepnd.
        ...
        {
          pose proof (TTCsent _ e n n) as M1h.
          rewrite i0 in M1h; simpl in M1h; repeat (autodimp M1h hyp); allrw; auto.
          exrepnd.

          apply (ind e'0 M1h0 (prop sn cmd)) in M1h1;
    ...
    - apply TTCdecided_iff in i; exrepnd; subst.
      ...
      + pose proof (TTCsent _ e sender n) as M1h.
        ...
        apply (ind e'0 M1h0 (prop sn c0)) in M1h1;
          [exrepnd; exists e'1; dands; eauto with eo|].
      ...
      + eapply in_quorum_from_vote in i8;[|exact xx].
        ...
        assert (e'1 ≺ e) as q.
        { apply localCausalOrderLe_subEventOrdering_implies in i0; simpl in i0.
          eauto 3 with eo. }

        apply (ind e'1 q (prop sn c0)) in M1h1;
    ...
    - exrepnd; subst.
      ...
      pose proof (poss_maj_in cmddeq cmds cmd) as xx; simpl in xx.
      rewrite i13 in xx; simpl in xx.
      destruct xx as [xx|xx]; subst.

      + pose proof (TTCsent eo e sender n) as M1h.
        rewrite i10 in M1h; simpl in M1h; repeat (autodimp M1h hyp); allrw; auto.
        exrepnd.
      ...
      + eapply in_quorum_from_vote in i9;[|exact xx].
        exrepnd.
        ...
        assert (e'1 ≺ e) as q.
        { apply localCausalOrderLe_subEventOrdering_implies in i1; simpl in i1.
          eauto 3 with eo. }
  ...
  Lemma TTCvalidity :
    forall (eo : EventOrdering),
      ...
      -> TTCall_not_byz eo
      -> TTChold_keys eo
      -> forall (e : Event) (p : Proposal) (c : name),
          In (MkDMsg c (TTCnotify p)) (output_system_on_event_ldata TTCsys eo e)
          ->
          exists e',
            e' ≼ e
            /\ trigger e' = TTCpropose p.
  ...
  Lemma SM_state_on_event_TTCnewVotersOpt_subset :
    forall (eo : EventOrdering) (e1 e2 : Event) lwm1 lwm2,
      e1 ⊏ e2
      -> SM_state_on_event TTCnewVotersOpt eo e1 lwm1
      -> SM_state_before_event TTCnewVotersOpt eo e2 lwm2
      -> subset_list_with_missing lwm1 lwm2.
  Proof.
    introv lc sme1 sme2.
    ...
    induction e2 as [e2 ind] using CausalOrderInd; introv lc sme2.
  ...
  Lemma SM_state_before_event_TTCnewRounds_update :
    forall (eo : EventOrdering) (e : Event) sn rn rn' cmd,
      SM_state_before_event (TTCnewRounds sn) eo e rn
      -> output_sm_on_event TTCroundInfo eo e = Some (Some (ballot (vround sn rn') cmd))
      -> rn < rn'
      -> SM_state_on_event (TTCnewRounds sn) eo e rn'.
  Proof.
    introv sm out ltrn.
  ...
  Lemma SM_state_on_event_TTCnewRounds_subset :
    forall (eo : EventOrdering) (e1 e2 : Event) sn rn1 rn2,
      e1 ⊏ e2
      -> SM_state_on_event (TTCnewRounds sn) eo e1 rn1
      -> SM_state_before_event (TTCnewRounds sn) eo e2 rn2
      -> rn1 <= rn2.
  Proof.
    introv lc sme1 sme2.
    unfold SM_state_on_event in sme1.
  ...
  Lemma TTCvotes_consistent :
    forall (eo : EventOrdering),
      ...
      -> TTCall_not_byz eo
      -> TTChold_keys eo
      -> forall (e1 e2 : Event) (vr : VotingRound) (cmd1 cmd2 : Cmd) (sender : Rep) (c1 c2 : name),
          In (MkDMsg c1 (TTCvote (vote (ballot vr cmd1) sender))) (output_system_on_event_ldata TTCsys eo e1)
          -> In (MkDMsg c2 (TTCvote (vote (ballot vr cmd2) sender))) (output_system_on_event_ldata TTCsys eo e2)
          -> cmd1 = cmd2.
  Proof.
    ...
    destruct H2h0 as [H2h0|H2h0].
    - rewrite H2h0 in H1h0.
      destruct H1h0 as [H1h0|H1h0]; exrepnd; ginv.
    - exrepnd; rewrite H2h0 in H1h0.
      destruct H1h0 as [H1h0|H1h0]; exrepnd; ginv.
      rewrite <- H1h2 in H2h9; ginv.
  Qed.
  ...
  Lemma SM_state_before_event_TTQuorum_has_at_least_n_implies_exists_votes :
    forall (eo : EventOrdering) (e : Event) vr n cmds names cmd k,
      SM_state_before_event (TTCQuorum vr n) eo e (cmds, names)
  ...
  Proof.
    intros eo.
    induction e as [e ind] using CausalOrderInd; introv sm hasn.
    unfold SM_state_before_event in *.
    rewrite state_sm_before_event_unroll in sm.
    ...
    - remember (state_sm_before_event (TTCQuorum vr n) eo (local_pred e)) as sop.
  ...
  Lemma TTCdecided_property :
    forall (eo : EventOrdering),
      ...
      -> TTCall_not_byz eo
      -> TTChold_keys eo
      -> forall (e : Event) (sn : SlotNum) (cmd : Cmd) (c : name),
          In (MkDMsg c (TTCdecided (prop sn cmd))) (output_system_on_event_ldata TTCsys eo e)
          ->
          exists (rn : RoundNum) (L : list Rep),
            ...
            forall r,
              ...
              exists e' dst,
                e' ≺ e
                /\ In (MkDMsg dst (TTCvote (vote (ballot (vround sn rn) cmd) r))) (output_system_on_event_ldata TTCsys eo e').
  Proof.
    ...
    - pose proof (SM_state_before_event_TTQuorum_has_at_least_n_implies_exists_votes
      ...
      + pose proof (sent e (MkAuthData (trigger e) [tt])) as M1h; simpl in M1h.
        autodimp M1h hyp;[].
        exrepnd.
        rewrite i9 in M1h0; simpl in M1h0.

        autodimp M1h0 hyp.

        {
          unfold verify_authenticated_data; simpl.
          pose proof (TTCkey_replicas eo e n r) as h.
          repeat (autodimp h hyp); exrepnd; eauto 3 with eo; allrw; simpl; auto.
        }
        ...
        destruct M1h0 as [M1h0|M1h0]; exrepnd;
          [|inversion M1h5 as [z]; clear M1h5;
            pose proof (nbyz e''0 r) as q;
            autodimp q hyp; ginv;tcsp];[].

        inversion M1h2 as [eql]; clear M1h2.

        assert (e'0 ≺ e) as q by (eauto 3 with eo).

        exists e'0 dst; dands; auto.

      + apply h0 in i; exrepnd; simpl in *.
        rewrite subEventOrdering_trigger_sub_eo2 in i0.
        apply localCausalOrder_subEventOrdering_iff in i14; simpl in *.

        pose proof (sent (sub_eo_event e'' e'0) (MkAuthData (trigger e'0) [tt])) as M1h; simpl in M1h.
        autodimp M1h hyp;[].
        exrepnd.
        rewrite i0 in M1h0; simpl in M1h0.

        autodimp M1h0 hyp.

        {
          unfold verify_authenticated_data; simpl.
          pose proof (TTCkey_replicas eo e'0 n r) as h.
          repeat (autodimp h hyp); exrepnd; eauto 3 with eo; allrw; simpl; auto.
          apply local_implies_loc in i14; allrw; auto.
        }
  ...
  Proof.
    intros eo.
    induction e as [e ind] using CausalOrderInd; introv sm icmds.
    unfold SM_state_before_event in *.
    rewrite state_sm_before_event_unroll in sm.
  ...
  Lemma TTCsimple_retry_property :
    forall (eo : EventOrdering),
      ...
      -> TTCall_not_byz eo
      -> TTChold_keys eo
      -> forall (e : Event) (vr : VotingRound) (cmd : Cmd) (c : name),
          In (MkDMsg c (TTCretry (ballot vr cmd))) (output_system_on_event_ldata TTCsys eo e)
          ->
          exists e' dst vr' r,
            vr = incVotingRound vr'
            /\ e' ≺ e
            /\ In (MkDMsg dst (TTCvote (vote (ballot vr' cmd) r))) (output_system_on_event_ldata TTCsys eo e').
  Proof.
    introv sent nbyz hkeys i.
...

    {
      ...
      rewrite i10 in M1h; simpl in M1h; repeat (autodimp M1h hyp); allrw; auto.
      exrepnd.
      exists e'0 dst (vround sn rn) sender; simpl; dands; auto.
    }

    eapply SM_state_before_event_TTQuorum_from_votes in i9;[|exact h].
    exrepnd.
    ...
    rewrite subEventOrdering_loc_as_loc in eqloc.
    dup i1 as lord.
    apply localCausalOrder_subEventOrdering_implies in lord; simpl in lord.

    pose proof (TTCsent eo e'0 sender0 n) as M1h.
    rewrite i2 in M1h; simpl in M1h; repeat (autodimp M1h hyp); allrw; auto.
    exrepnd.

    exists e'1 dst (vround sn rn) sender0; simpl; dands; eauto 3 with eo.
  Qed.
  ...
  Lemma TTCretry_property_local :
    forall (eo    : EventOrdering)
           ...
           (names : list Rep)
           (cmd   : Cmd),
      SM_state_before_event (TTCQuorum vr n) (subEventOrdering e0) (mkSubOrderingLe p) (cmds, names)
      ->
      forall (i : nat),
        i < length cmds
        ->
  ...
  Proof.
    intros eo.
    induction e as [e ind] using CausalOrderInd; introv sm lti.
    unfold SM_state_before_event in *.
    rewrite state_sm_before_event_unroll in sm.
  ...
  Lemma TTCretry_property :
    forall (eo    : EventOrdering)
      ...
      -> TTChold_keys eo
      -> loc e = TTCreplica n
      -> SM_state_before_event (TTCQuorum vr (TTCreplica n)) (subEventOrdering e0) (mkSubOrderingLe p) (cmds, names)
      ->
      forall (i : nat),
        i < length cmds
        ->
        exists (e' : Event) (dst : name),
          e' ≺ e
          /\ In (MkDMsg dst (TTCvote (vote (ballot vr (nth i cmds cmd)) (nth i names replica0)))) (output_system_on_event_ldata TTCsys eo e').
  Proof.
    ...
    exrepnd.
    exists e'0 dst; dands; eauto 3 with eo.
  Qed.
  ...
  Proof.
    intros eo.
    induction e as [e ind] using CausalOrderInd; introv sm.
    unfold SM_state_before_event in *.
    rewrite state_sm_before_event_unroll in sm.
    destruct (dec_isFirst e) as [d1|d1]; simpl in *; ginv; simpl in *; tcsp.
  ...
  Lemma unanimous_implies_consistent_vote :
    forall (eo  : EventOrdering)
      ...
      -> (forall r : Rep,
             ...
             exists e' dst,
               (e') ≺ (e) /\
               In (MkDMsg dst (TTCvote (vote (ballot (vround sn rn) cmd) r)))
                  (output_system_on_event_ldata TTCsys eo e'))
      -> forall (k      : nat)
                (e'     : Event)
                (dst    : name)
                (sender : Rep)
                (c      : Cmd),
          In (MkDMsg dst (TTCvote (vote (ballot (vround sn (addRoundNum rn (S k))) c) sender)))
             (output_system_on_event_ldata TTCsys eo e')
          -> c = cmd.
  Proof.
    introv sent nbyz hkeys norep len imp.
    induction k as [? indk] using comp_ind.
    induction e' as [e' inde] using CausalOrderInd; introv i.
    ...
    destruct i9 as [i9|i9]; exrepnd; ginv.

    - pose proof (TTCsent _ e' n n) as M1h.
      rewrite i9 in M1h; simpl in M1h.
      repeat (autodimp M1h hyp); allrw; auto.

      exrepnd.

      applydup TTCsimple_retry_property in M1h1 as M3h1; auto.
      exrepnd.
      destruct vr'; simpl in *.

      inversion M3h0; subst; clear M3h0.
      match goal with
      ...
      destruct k;[|apply indk in M3h1; auto].
      clear indk M3h1.

      (* Now we have to prove that this retry [M1h1] couldn't have happened
         because of the unanimous quorum L *)
      apply TTCretry_iff in M1h1; exrepnd.

      inversion M1h1; subst; clear M1h1.
      match goal with
      | [ H : _ + _ = _ |- _ ] => rename H into eqp
      end.
                 ...
                 -> nth i (cmd3 :: cmds) cmd3 = cmd) as xx.
      {
        ...
        - apply quor3 in z2.
          ...
          eapply TTCvotes_consistent in z2; try (exact M4h1); auto.

        - pose proof (q i) as zz; autodimp zz hyp; try omega;[].
          apply quor3 in z2.
          apply imp in z2; exrepnd.
          eapply TTCvotes_consistent in z2; try (exact zz1); auto.
      }
                 ...
                 /\ contains_only cs cmd) as exs.
      {
        ...
        - introv i.
          ...
          apply xx; auto.
          { apply in_find_positions_implies_lt in i0; simpl in *; try omega. }
          { eapply in_find_positions_implies_nth_in in i0; exact i0. }
      }
      ...
      rewrite M1h13 in uuu.
      omega.

    - destruct v; simpl in *; subst.

      pose proof (TTCsent _ e' sender n) as M4h.
      rewrite i9 in M4h; simpl in M4h.
      repeat (autodimp M4h hyp); allrw; auto.
      exrepnd.
      apply inde in M4h1; auto.
  Qed.
  ...
  Lemma TTCdecided_agreement :
    forall (eo : EventOrdering),
      ...
      -> TTCall_not_byz eo
      -> TTChold_keys eo
      -> forall (e1 e2 : Event) (sn : SlotNum) (cmd1 cmd2 : Cmd) (c1 c2 : name),
          In (MkDMsg c1 (TTCdecided (prop sn cmd1))) (output_system_on_event_ldata TTCsys eo e1)
          -> In (MkDMsg c2 (TTCdecided (prop sn cmd2))) (output_system_on_event_ldata TTCsys eo e2)
          -> cmd1 = cmd2.
  Proof.
    ...
    - subst.
      ...
      eapply TTCvotes_consistent in q1; try (exact q2); auto.
  ...
  Lemma TTCagreement :
    forall (eo : EventOrdering),
      ...
      -> TTChold_keys eo
      -> forall (e1 e2 : Event) (sn : SlotNum) (cmd1 cmd2 : Cmd) (c1 c2 : name),
          In (MkDMsg c1 (TTCnotify (prop sn cmd1))) (output_system_on_event_ldata TTCsys eo e1)
          -> In (MkDMsg c2 (TTCnotify (prop sn cmd2))) (output_system_on_event_ldata TTCsys eo e2)
          -> cmd1 = cmd2.
  Proof.
    ...
    eapply TTCdecided_agreement in M1h1; try (exact M2h1); auto.
  Qed.
